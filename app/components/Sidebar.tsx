...
export function Sidebar({ user }: SidebarProps) {
    ...
    return (
        <>
            ...
            <aside 
                className={`fixed lg:relative inset-y-0 left-0 z-50 bg-card border-r border-card-border flex flex-col h-full transform transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] shrink-0 ${isOpen ? 'w-64 translate-x-0 shadow-2xl lg:shadow-none' : 'w-64 -translate-x-full lg:translate-x-0 lg:w-20'}`}
            >
            ...
            <div className={`p-4 border-t border-card-border ${!isOpen ? 'lg:p-2' : ''}`}>
                <div className={`flex flex-wrap justify-center gap-x-3 gap-y-1 mb-2 text-[10px] text-muted-text/60 transition-all duration-200 ${isOpen ? 'opacity-100' : 'lg:hidden opacity-0 h-0 overflow-hidden mb-0'}`}>
